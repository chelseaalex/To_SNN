...
"""
class ViT(nn.Module):
    def __init__(self, image_size=32, patch_size=8, num_classes=10, dim=64, depth=6, heads=8, mlp_dim=128):
        super().__init__()

        assert image_size % patch_size == 0, 'Image dimensions must be divisible by the patch size.'
        num_patches = (image_size // patch_size) ** 2
        patch_dim = 3 * (patch_size ** 2)  # Number of features in each patch

        self.patch_size = patch_size
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))
        self.patch_to_embedding = nn.Linear(patch_dim, dim)  # Adjusted to match patch_dim and dim
    ...
    def forward(self, img):
        ...
        x = x.flatten(2)
        print(x.shape)
        # Embedding patches and adding position embeddings
...
def get_cifar10_loader(batch_size, train_batch=None, num_workers=0, conversion=False, distributed=False):
...
def train(net, train_iter, test_iter, optimizer, scheduler, device, num_epochs, losstype='mse'):
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best:
            best = test_acc
            torch.save(net.state_dict(), './CIFAR10_vit.pth')
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load("./CIFAR10_vit.pth", map_location=device))
    net = net.to(device)
